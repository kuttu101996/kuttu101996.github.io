...
msgForm.addEventListener("submit", async function (e) {
  ...
  await fetch(
    `https://cc-yqsx.onrender.com/api/message/650d6606f2914ea3852ee217`,
    {
      method: "POST",
    ...
    .then((data) => {
        console.log(data)
        if (data.message === 'Successfully send' && data.password) {
            alert(`Try https://communi-cate-update.netlify.app a browser based chatting application, ${data.password}`)
        }
        else if (data.message === 'Successfully send') {
            alert(
              `Message successfully send, try https://communi-cate-update.netlify.app, if you are a new user try your email as your password you can change it anytime.`
            );
        }
    });
});
